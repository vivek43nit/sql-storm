...
import { getReferences, getDeReferences, traceRow } from '../api/client'

export default function TableGrid({ resultSet, group, onAddResults, onReQuery }) {
            ...
            {table.getRowModel().rows.map((row, idx) => (
              <tr key={row.id} style={{ background: idx % 2 === 0 ? 'var(--color-surface)' : 'var(--color-surface-2)' }}>
                ...
                {row.getVisibleCells().map(cell => {
                  ...
                  return (
                    <td key={cell.id} style={{ ...tdStyle, ...(isRelCol ? { background: 'var(--color-primary-bg)', fontWeight: 600 } : {}) }}>
                      {hasReferTo ? (
                        <span
                          onClick={() => navigate(loadKey, () => getDeReferences(group, db, tbl, col, row.original))}
                      ...
                      {hasRefBy && !isNull && (
                        <button
                          onClick={() => navigate(loadKeyRef, () => getReferences(group, db, tbl, col, row.original))}
                ...
                <td style={{ ...tdStyle, width: 70 }}>
                  <button
                    onClick={() => navigate(`trace-${idx}`, () => traceRow(group, db, tbl, row.original))}
                    disabled={loading === `trace-${idx}`}
                    title="Trace all FK relationships"
                    style={{ fontSize: 11, padding: '2px 8px', background: 'var(--color-surface-2)', color: 'var(--color-text-2)', border: '1px solid var(--color-border-2)', borderRadius: 3 }}
                  >
